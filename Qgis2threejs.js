...
// Highlight buildings based on selected timeline item
document.querySelectorAll('.timeline-item').forEach(item => {
	item.addEventListener('click', function () {
	  const year = this.getAttribute('data-year');
	  if (yearMessages[year]) {
		showPopupMessage(yearMessages[year]);
	  }
	  highlightBuildings(year);
	});
  });
  
  function highlightBuildings(year) {
	// Define the year ranges for each decade, etc.
	const buildingData = {
	  "1890": ["Kaya Rosa"], // Replace with actual building names/IDs for 1890s
	  "1900": ["Student Affairs Building"],
	  "1910": ["Old Arts Building", "Old Chemsitry Building", "Old College House"], 
	  "1920": ["JJ Theron Lecture Hall", "Masker Theatre", "Lier Theatre", "Old Agriculture Building", "Agricultural Annexe", "Zoology Building", "Chapel", "Drama Building", "Club Hall"],// Replace with actual building names/IDs for 1900s
	  "1930": ["Communication Pathology Building", "Mathematics Building", "Old Merensky Library"],
	  "1940": ["Botany Building", "Building Sciences Building", "Chemistry Building", "Agricultural Sciences Building", "Monastery Hall", "Visual Arts Building", "Stoneman Building", "Vetman Building", "Bateman Building"], 
	  "1950": ["Theology Building", "Mineral Sciences Building", "AE Du Toit Auditorium and Annexe", "Heavy Machinery Laboratories", "Aula and Rautenbach Hall", "Music Building", "Musaion and Amphitheatre", "Chancellor's Building"], 
	  "1960": ["Engineering 2 Building", "Geography Building", "Natural Sciences 2 Building", "Van Der Graaf Accelrator", "Administration Building", "Visitor's Reception"], 
	  "1970": ["Merensky 2 Library", "Market Services Building", "CEFIM Building", "Student Health Building", "Engineering 1 Building", "Technical Services Building", "Sci-Enza Centre", "Humanities Building"], 
	  "1980": ["Tukkiewerf", "Centenary Building", "Law Clinic", "Information Technology Building", "Natural Sciences 1 Building", "Sanlam Auditorium/Lecture Halls", "Conference Centre", "Economic Menagement Sciences Building"], 
	  "1990": ["Akanyang", "Student Centre Building", "Graduate Centre", "FABI 1"], 
	  "2000": ["Student Services Centre Building", "FABI 2", "Law Building", "Student Affairs and Accomodation"], 
	  "2010": ["Engineering 3 Building", "Centre for Responsible Leadership", "Thuto Building", "Plant Science Complex", "Transformation Office", "Student Gallery and Arts Square"],
	  // Continue for other years...
	};
	  
	// Retrieve the relevant buildings for the selected year
	const buildingsToHighlight = buildingData[year] || [];
  
	// Reset highlights before applying new highlights
	resetHighlight();
  
	// Highlight selected buildings
	buildingsToHighlight.forEach(buildingId => {
	  const building = app.scene.getObjectByName(buildingId); // Assuming buildings have unique names in the scene
	  if (building) {
		building.material.emissive.setHex(0xff0000); // Change to desired highlight color
	  }
	});
  }
  
  function resetHighlight() {
	// Assuming a specific material color to reset to (e.g., gray)
	app.scene.traverse(function (object) {
	  if (object.isMesh) {
		object.material.emissive.setHex(0x333333); // Original color
	  }
	});
  }
